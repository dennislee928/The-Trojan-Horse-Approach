...
if ! koyeb_cli domains get "${KOYEB_DOMAIN}" >/dev/null 2>&1; then
  info "creating custom domain ${KOYEB_DOMAIN} and attaching it to ${KOYEB_APP_NAME}"
  koyeb_cli domains create "${KOYEB_DOMAIN}" --attach-to "${KOYEB_APP_NAME}"
else
...
if [[ "${KOYEB_DOMAIN_REFRESH:-0}" == "1" ]]; then
  ...
  koyeb_cli domains refresh "${KOYEB_DOMAIN}"
fi

info "current domain metadata:"
koyeb_cli domains get "${KOYEB_DOMAIN}" -o json
